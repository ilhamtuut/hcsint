@extends('layouts.backend',['page'=>'convert','active'=>'topup'])
    ...
    <div class="col-lg-12 mb-30">
        ...
        <div class="ibox-home bg-boxshadow">
            ...
            <div class="ibox-content">
                <div class="row">
                    <div class="col-lg-6"><h5 class="text-warning">Convert Cash Wallet to Topupclingg</h5></div>
                    <div class="col-lg-6 text-right"><a class="btn btn-warning rounded-0" href="{{route('convert.history_topup')}}"><i class="fa fa-history"></i> History</a></div>
                </div>
            <hr>
            <p>Balance (CW) : {{$balance}}</p>
            {{-- <p>Price AV : ${{$price}}</p> --}}
            <form class="form-horizontal form-label-left" action="{{route('convert.sendTopup')}}" method="POST">
                @csrf
                <div class="form-group">
                    <label class="control-label">Account Topupclingg</label>
                    <input id="account" name="account" class="form-control" placeholder="Account Topupclingg" type="text">
                    <label class="control-label mb-0" id="accountName"></label>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input id="receive" class="form-control" readonly placeholder="Receive (IDR)" type="text">
                </div>
                {{-- <div class="form-group">
...
@section('script')
    <script type="text/javascript">
        ...
        $('#account').on('keyup change',function () {
    		var user = $(this).val();
            $.ajax({
                url:'{{ url('/convert/check') }}/' + user,
                type:'GET',
                success:function(data) {
                    $('#accountName').html('');
                    if(data.success){
                        $('#accountName').html('Account Name : ' + data.data.name);
                    }
                },
            });
        });
