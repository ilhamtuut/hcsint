@extends('layouts.backend',['page'=>'convert','active'=>'history'])

@section('header')
  <h4 class="font-color-purple"><i class="fa fa-exchange"></i> <span>History Convert</span></h4>
@endsection
                          ...
                          @forelse ($data as $value)
                            @php
                                ...
                                if(isset($json->account)){
                                    $account = '['.ucfirst($json->account).']';
                                }
                            @endphp
                            <tr>
                              <td>{{++$i}}</td>
                              <td>{{date('d F Y H:i:s', strtotime($value->created_at))}}</td>
                              <td>{{$value->description}} {{$account}}</td>
